...
int main(){
    ...
    pthread_create(&timerThread, NULL, TimerThread, (void *)timer);
    pthread_create(&arrivalThread, NULL, ArrivalThread, (void *)arrivals);
    pthread_create(&IOthread, NULL, IOsim, (void *)devIO);
            ...
            if (io->DeviceQ->size > 0) {
                ...
                if(currentProcess->currentBurst==currentProcess->numberOfBursts){ //Process Complete
                    ...
                    enqueueProcess(core->completedQ,currentProcess);
                        ...
                        currentProcess->lastRequeTime = *(core->t);
                        enqueueProcess(core->readyQ,currentProcess);
                        currentProcess->currentBurst++;
                }
                else{
                    printf("Process error for pid%d!!\n",currentProcess->pid);
